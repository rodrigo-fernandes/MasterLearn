var Player = function($container) {
	...
	function exibirPergunta(pergunta) {
		...
		setTimeout(function() {
			...
			for (var i = 0; i < respostas.length; i++) {
				...
				if (game.getCategoria() == Categoria.INTERDISCIPLINAR) {
					drawImage(canvasResposta, game.getCategoria(), respostas[i]);
	...
	function exibirBarraProgresso() {
		...
		var tempoAtual = 0;
		var tempoTotal = 1000000; // Ajusta o tempo da barre de progresso lateral
		var interval = setInterval(function() {
			tempoAtual += 100;
	...
	function fimDeJogo(resultado) {
		...
		if (resultado) {
			...
			wrongAudio[0].play();
			resultadoContainer.text("Tente novamente");
